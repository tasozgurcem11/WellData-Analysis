...
for id in API:
    ...
    #Dropping values before 2014
    old_rows = new_df[ (new_df["ReportYear"].astype(int) < 2018)  ].index
...
for df0 in new_list:
    ...
    df0 = df0.groupby(['PRODMMYYYY']).agg(**{"TotalOil": ('TotalOil', 'max'), "TotalGas": ('TotalGas', 'max'), "API": ('API', 'max'), "ReportMonth": ('ReportMonth', 'max'), "ReportYear": ('ReportYear', 'max'), "WellName": ('WellName', 'max'), "STR": ('STR', 'max'),  "FirstProd": ('FirstProd', 'max') , "CompletionDate": ('CompletionDate', 'max') , "WellboreProfile": ('WellboreProfile', 'max'),"CurrentOperator": ('CurrentOperator', 'max'), "WellStatus": ('WellStatus', 'max')}).reset_index()
    df0 = df0.sort_values(by=["ReportYear", "ReportMonth"], ascending=[False, False])
    
    if df0.shape[0] >= 4:
        df0_list.append(df0)
        
print(df0_list[53].shape)
...
print(general_list3[120].shape)
print(general_list3[11].loc[0, "STR"])
...
updated_cols = ['STR', 'API', 'PRODMMYYYY', 'TotalOil', 'TotalOilSum', 'TotalGas', 'GasStatus', 'OilStatus', 'WellName', 'WellboreProfile', 'WellStatus','CurrentOperator', 'CompletionDate', 'FirstProd'] 
...
updated_cols2 = ['STR', 'API', 'PRODMMYYYY', 'TotalOil', 'TotalOilSum', 'TotalGas', 'GasStatus', 'OilStatus', 'WellName', 'WellboreProfile', 'WellStatus','CurrentOperator', 'CompletionDate', 'FirstProd'] 
final_df4 = final_df3[updated_cols2]
final_df4.sort_values(['STR', 'API'], ascending=[True, True], inplace=True)
#final_df4.to_csv(r'final5.csv', index = False)